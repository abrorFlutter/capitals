...
class DatabaseHelper {
  ...
  Future<List<Map<String, Object?>>?> getWordMap({String? word, bool? isCity}) async {
    final data = await db;
    ...
    if (word == null) {
      ...
      result = await data?.query(
        tableName, where: isCity! ? '$colCapital LIKE ?':'$colCountry LIKE ?', whereArgs: ["$word%"]
      );
    }
    return result;
  }
  ...
  Future<List<Word>> getWordLike(String word,bool isCity) async {
    final List<Map<String, Object?>>? wordMap = await getWordMap(word: word,isCity: isCity);
    final List<Word> words = [];
    wordMap?.forEach((element) {
      words.add(Word.fromMap(element));
    });
